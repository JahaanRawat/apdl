-- Canonical ClickHouse schema baseline for fresh APDL installations.

-- Upgrade and retained-data backfill operations are intentionally absent from this baseline.
...
TTL toDate(received_at) + toIntervalMonth(12)
...
CREATE TABLE IF NOT EXISTS experiment_event_deliveries
(
    `project_id` String,
    `message_id` String,
    `event_type` LowCardinality(String),
...
ENGINE = ReplacingMergeTree(received_at)
PARTITION BY project_id
ORDER BY (project_id, source_stream, source_stream_id)
TTL toDate(received_at) + toIntervalMonth(12)
SETTINGS index_granularity = 8192
;
...
(
    ...
    `source_stream_id_seq` UInt64,
    `event_date` Date DEFAULT toDate(received_at),
    INDEX idx_identity_alias_anonymous_id anonymous_id TYPE bloom_filter(0.01) GRANULARITY 4
)
ENGINE = ReplacingMergeTree(received_at)
PARTITION BY project_id
...
CREATE MATERIALIZED VIEW IF NOT EXISTS feature_flag_exposures_mv TO feature_flag_exposures
(
    `project_id` String,
    `message_id` String,
    `flag_key` String,
...
FROM events
WHERE (event_name = '$feature_flag_exposure') AND (JSONExtractString(properties, 'reason') IN ('rule_match', 'fallthrough'))
;

CREATE MATERIALIZED VIEW IF NOT EXISTS frontend_health_events_mv TO frontend_health_events
(
    `project_id` String,
    `message_id` String,
    `event_name` LowCardinality(String),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS identity_alias_assertions_mv TO identity_alias_assertions
(
    `project_id` String,
    `message_id` String,
    `anonymous_id` String,
...
AS SELECT
    ...
    toDate(received_at) AS event_date
FROM events
WHERE (event_type = 'identify') AND (user_id != '') AND (anonymous_id != '')
;

CREATE MATERIALIZED VIEW IF NOT EXISTS experiment_event_deliveries_mv TO experiment_event_deliveries
(
    `project_id` String,
    `message_id` String,
    `event_type` LowCardinality(String),
...
CREATE VIEW IF NOT EXISTS resolved_identity_aliases
(
    `project_id` String,
    `anonymous_id` String,
    `resolved_user_id` String,
    `has_conflict` UInt8,
    `first_identified_at` DateTime64(3),
    `last_identified_at` DateTime64(3)
)
AS SELECT
    project_id,
    anonymous_id,
