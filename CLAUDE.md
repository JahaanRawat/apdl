...
- PR review: follow `docs/agent-workflows/pr-review.md` when the user asks to review a PR, review a diff, review changes before pushing, or check whether a branch is safe to merge. It is the canonical graph-aware review workflow for the monorepo, wrapped as the `pr-review` skill.
